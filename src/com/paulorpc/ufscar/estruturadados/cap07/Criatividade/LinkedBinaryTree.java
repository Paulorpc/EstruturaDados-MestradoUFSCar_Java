...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	...
	/** métodos PAULO exericicios **/
	
	
	/**
	 * Exercicio 2
	 */
	private int preorderSubtree2(Position<E> p, List<Position<E>> snapshot) {
		int h = 0;
		snapshot.add(p); 
		
		for (Position<E> c : children(p))
			h = Math.max(h, 1 + preorderSubtree2(c, snapshot));
	 ...
	 * Exercicio 2 e 4
	 */
	private void preorderSubtree3(Position<E> p) {

		System.out.print(p.getElement().toString() + " .... Prof: " + depth(p) + "  Alt: " +height(p) + "\n");
	...
	public void printPreorder2() {
		...
		if (!isEmpty())
			h = preorderSubtree2(root(), snapshot);
		
		for(Position<E> pos: snapshot)
	 ...
	 * Exercicio 2
	 */
	public void printPreorder3() {
		if (!isEmpty())
			preorderSubtree3(root());
	}
	
	/**
	 * Exercicio 3
	 */
	public void preOrderPaulo(Position<E> p) {
	 ...
	 * Exercicio 3
	 */
	public void posOrderPaulo(Position<E> p) {
		for(Position<E> pos : children(p))
			posOrderPaulo(pos);
		System.out.println(p.getElement().toString());
	}
	 ...
	 * Exercicio 3
	 */
	public void inOrderPaulo(Position<E> p) {
		...
		if( right(p) != null) 
			inOrderPaulo(right(p));
	}
	
	/**
	 * Exercicio 5
	 */
	public String preOrderIdentado(Position<E> p) {
		
		String ret = p.getElement().toString();
		if (isInternal(p)) ret += " ( \n";
		
		for(Position<E> pos : children(p)) {
			ret += "          ".substring(0, depth(pos));
			ret += preOrderIdentado(pos) +"\n";
		}
		ret += "          ".substring(0, depth(p));
		if (isInternal(p)) ret += " ) ";
		
		return ret;
	}
	
	public void eulerPrintExpressão(Position<E> p) {
		
		if(isInternal(p))
			System.out.print("(");
		
		if (left(p) != null) {
			eulerPrintExpressão(left(p));
		}
			
		System.out.print(p.getElement().toString());
		
		if (right(p) != null) {
			eulerPrintExpressão(right(p));
		}
	...
	// FUNCIONA, MAS NÃO SEI SE CUPRE AS REGRAS DO CONTADOR DE EULER.	
	public int eulerNroDescendentes(Position<E> p, int cont) {
		if (left(p) != null) {
			cont = eulerNroDescendentes(left(p), cont+1);
		}
		
		if (right(p) != null) {
			cont = eulerNroDescendentes(right(p), cont+1);
		}
		return cont;
	}
	
	// CONSIDERANDO ARVORE BINARIA PROPRIA (BALANCEADA)
	public int eulerNroNiveis(Position<E> p) {
		int cont = eulerNroDescendentes(p, 0);
		
		//log x base 2 = log x / log 2
		cont = (int) Math.floor( Math.log10(cont)/Math.log10(2) );
		return cont;
	}

	
	/** fim métodos PAULO exericicios **/
