...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	...
	public int posOrderOperacao(Position<E> p, Stack<String> s) {

		Integer res=0;
		String[] ops = {"+", "-", "/", "*"};
		
		for(Position<E> pos : children(p))
			posOrderOperacao(pos, s);
		
		if(isExternal(p))
			s.push( p.getElement().toString() );
		...
		if(isInternal(p)) {
			...
			if( "+".equals(p.getElement().toString()) ) res = op1 + op2;
			else if( "-".equals(p.getElement().toString()) ) res = op1 - op2; 
			else if( "/".equals(p.getElement().toString()) ) res = op1 / op2; 
			else if( "*".equals(p.getElement().toString()) ) res = op1 * op2; 
			s.push(res.toString());
		}
