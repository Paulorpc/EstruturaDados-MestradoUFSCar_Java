...
public class ArrayList<E> implements List<E> {
	...
	@SuppressWarnings("unchecked")
	public ArrayList<E> cardShufle() {
		
		ArrayList<E> newData = new ArrayList<>(2*size);
		int sizeL1 = 0;
		int sizeL2 = 0;
		
		if(size % 2 == 0) sizeL1 = sizeL2 = size/2;
		else {
			sizeL1 = (size+1)/2;
			sizeL2 = size-sizeL1;
		}
		
		E[] l1 = (E[]) new Object[size];
		E[] l2 = (E[]) new Object[size];
		
		for(int i=0; i < sizeL1 ; i++) {
			l1[i] = data[i];
		}
		
		for(int i=0, j=sizeL1; i < sizeL2 && j < size ; i++, j++) {
			l2[i] = data[j];
		}
