...
public class LinkedPositionalList<E> implements PositionalList<E> {
	...
	/*
	 * Implementação do método para exercício
	 * @see Ex7_39_Ingles.PositionalList#moveToFront()
	 */
	...
	/**
	 * Método de troca dos paramentros sem perder referência original. Esta
	 * criando novos nós para auxiliar
	 * 
	 * @param Position p
	 * @param Position q
	 */
	public void swap(Position<E> p, Position<E> q) {
		// Retorna os nodes dos position
		Node<E> nodeP = validate(p);
		Node<E> nodeQ = validate(q);

		// Recupera o predecessor e sucessor de P
		Node<E> predP = nodeP.getPrev();
		Node<E> sucP = nodeP.getNext();

		// Recupera o predecessor e sucessor de Q
		Node<E> predQ = nodeQ.getPrev();
		Node<E> sucQ = nodeQ.getNext();

		// Inverte o redirecionamento dos links de Q e P para os sucessores e
		// antecessores um do outro
		nodeP.setNext(sucQ);
		nodeP.setPrev(predQ);
		nodeQ.setNext(sucP);
		nodeQ.setPrev(predP);

		// Inverte os links dos predecessores e antecessores de Q e P para P e Q
		predP.setNext(nodeQ);
		sucP.setPrev(nodeQ);
		predQ.setNext(nodeP);
		sucQ.setPrev(nodeP);

		// Retornq os position para a referência original.
		p = position(nodeP);
		q = position(nodeQ);
	}
